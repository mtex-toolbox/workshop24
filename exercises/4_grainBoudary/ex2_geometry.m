...
ebsd = EBSD.load('newsubset.h5oina','interface','h5oina','fullDataset')
...
xyzPlot = plottingConvention;
xyzPlot.outOfScreen = -zvector;
xyzPlot.east = xvector;
setMTEXpref('xyzPlotting',xyzPlot);

%%

plot(ebsd,ebsd.orientations,'region',[-500 -100 200 400])


hold on
plot(grains.boundary,'lineWidth',1.5,'region',[-500 -100 200 400])
hold off

hold on
plot(grains.innerBoundary,'lineWidth',1,'region',[-500 -100 200 400],'linecolor','blue')
hold off


%%

[grains ebsd.grainId] = calcGrains(ebsd('indexed'),'angle',[10 1]*degree)
