...
\begin{frame}[fragile]
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}
      \item describe the relative alignment of two crystal
        lattices with respect to each other
      \item if the  lattices are of same phase it is the rotation
        that rotates one lattice onto the other.
      \item if the lattices of different phase it is the rotation between the
        corresponding orthonormal reference frames
      \item is the coordinate transform from lattice A coordinates into
        lattice B coordinates
      \item is subject to the symmetries of lattice A as well as lattice B
      \end{itemize}
  ...
  \end{column}
    ...
    \begin{column}{0.45\textwidth}
      ...
      \begin{overlayarea}{\textwidth}{0.85\textheight}
        \begin{onlyenv}<1->
          \centering \includegraphics[width=0.8\textwidth]{pic/japanTwin}
...
plot(mori * cS)
    ...
    %   examples:
    %   \begin{itemize}
    %   \item neighboring pixels \\
    %     $\rightarrow$ texture gradient, KAM
    %   \item pixel to grain mean orientation
    %     $\rightarrow$ GOS
    %   \item grain boundary misorientations
    %   \item twinning misorientations
    %   \item csl misorientations
    %   \item parent to child orientation relationships
    %   \end{itemize}
    % \end{onlyenv}
  \end{overlayarea}
\end{column}
    ...
    \begin{columns}
      \begin{column}{0.46\textwidth}
        \begin{overlayarea}{\textwidth}{0.85\textheight}
          ...
          \vspace{-0.2cm}
          \begin{lstlisting}[style=input]
ebsd.gradientX, ebsd.KAM
  \end{lstlisting}

  \begin{onlyenv}<2->
    grain boundary misorientations \vspace{-0.2cm}
  \begin{lstlisting}[style=input]
grains.boundary.misorientation
  ...
  \begin{onlyenv}<3->
    pixel to grain mean orientation
\vspace{-0.2cm}
        \begin{lstlisting}[style=input]
ebsd.calcGROD(grains)
grains.GOS
  \end{lstlisting}
\end{onlyenv}
...
\begin{onlyenv}<4->
 ...
 \vspace{-0.2cm}
 \begin{lstlisting}[style=input]
xi = Miller(1,0,-1,1,CS)
orientation.byAxisAngle(xi,pi)
  \end{lstlisting}
\end{onlyenv}

\begin{onlyenv}<5->
  common site lattice misorientations
 \vspace{-0.2cm}
  \begin{lstlisting}[style=input]
mori = CSL(3,cs)
  \end{lstlisting}
...
\begin{column}{0.52\textwidth}
  \begin{overlayarea}{\textwidth}{0.85\textheight}
    \vspace{-0.75cm}
    \begin{onlyenv}<1>
\includegraphics[height=7cm]{pic/gradientX}
\includegraphics[height=7cm]{pic/gradientY}
              \begin{lstlisting}[style=input]
ori = ebsd.orientation;
angle(ori(:,1:end-1),ori(:,2:end))
\end{lstlisting}
\end{onlyenv}


\begin{onlyenv}<2>
  \includegraphics[height=7cm]{pic/boundaryAngle}
  \begin{lstlisting}[style=input]
gB   = grains.boundary
mori = gB.misorientation
plot(gB, mori.angle ./ degree)
  \end{lstlisting}
\end{onlyenv}

  \begin{onlyenv}<3>
    \includegraphics[height=7cm]{pic/mis2mean}
    \includegraphics[height=7cm]{pic/gos}
    \begin{lstlisting}[style=input]
...
\begin{onlyenv}<4>
  \vspace{1cm}
  \begin{center}
    \includegraphics[width=0.9\textwidth]{pic/japanTwin}
  \end{center}
\end{onlyenv}
...
\begin{onlyenv}<5>
  \begin{center}
    \vspace{-0.5cm}
    \includegraphics[width=0.8\textwidth]{pic/csl3}
  \end{center}
\end{onlyenv}
...
\end{lstlisting}
  ...
  \vspace{-.2cm}
  \begin{lstlisting}[style=input]
oAlpha = orientation.byEuler(135, 90, 55, csAlpha)
inv(oAlpha) * r
\end{lstlisting}
  \begin{onlyenv}<2>
    \vspace{-.3cm}
    \begin{lstlisting}[style=output]
oAlpha = /+orientation+/ (/+Ti (alpha)+/ -> xyz)
...
\end{lstlisting}
  ...
  Hence, $\text{\bf inv}(\mathbf{aAlpha}) * \mathbf{oBeta}$ converts beta
  coordinates into alpha coordinates
  \vspace{-.2cm}
    \begin{lstlisting}[style=input]
inv(oAlpha) *  oBeta * Miller(1,1,0,csBeta)
  \end{lstlisting}
  \begin{onlyenv}<3>
    \vspace{-.3cm}
    \begin{lstlisting}[style=output]
...
\begin{frame}[fragile]
  ...
  \begin{overlayarea}{\textwidth}{0.85\textheight}
    ...
    The \alert{misorientation} {\bf beta2alpha} transforms Beta coordinates into Alpha coordinates
\vspace{-0.1cm}
\begin{lstlisting}[style=input]
beta2alpha = inv(oAlpha) * oBeta
beta2alpha * Miller(1,1,0,csBeta)
\end{lstlisting}
...
\end{onlyenv}
  ...
  \pause
  \medskip
its inverse transforms Alpha coordinates into Beta
coordinates
\vspace{-0.2cm}
\begin{lstlisting}[style=input]
alpha2beta = inv(beta2alpha)
alpha2beta * Miller(0,0,0,1,csAlpha)
\end{lstlisting}
  \begin{onlyenv}<2>
  \vspace{-.3cm}
  \begin{lstlisting}[style=output]
alpha2beta = /+misorientation+/ (/+Ti (alpha)+/ -> /+Ti (beta)+/)
...
\texttt{beta2alpha} is close to the Burger orientation relationship
between the hexagonal alpha and the cubic beta phase.
\vspace{-0.2cm}
\begin{lstlisting}[style=input]
...
\end{lstlisting}
  \vspace{-.3cm}
  \begin{lstlisting}[style=output]
(110) || (0001)   [1-11] || [-12-10]   error: 0.26
 \end{lstlisting}

\pause
\medskip

define the Burger orientation relationship exactly
\vspace{-0.2cm}
\begin{lstlisting}[style=input]
beta2alpha = orientation.map(...
  Miller(1,1,0,csBeta,'hkl'),Miller(0,0,0,1,csAlpha,'hkl'),...
...
\begin{frame}[fragile]
  \frametitle{Symmetrically Equivalent Misorientations}

  \begin{itemize}
  \item \texttt{sA} - symmetry operation from \texttt{csAlpha}
  \item \texttt{sB} - symmetry operation from \texttt{csBeta}
  \item \texttt{beta2alpha} - misorientation from beta to alpha
  \item symmetrically equivalent misorientations:\\\texttt{sA * beta2alpha * sB}
  \item number of symmetric  misorientations:
    \begin{itemize}
    \item 2 Laue groups: \texttt{|csAlpha| * |csBeta| / 2}
    \item otherwise: \hspace{0.51cm} \texttt{|csAlpha| * |csBeta|}
    \end{itemize}
  \end{itemize}

\begin{lstlisting}[style=input]
beta2alpha.symmetrise
\end{lstlisting}
\vspace{-0.2cm}
\begin{lstlisting}[style=output]
ans = /+misorientation+/ (/+Ti (beta)+/ -> /+Ti (alpha)+/)
  size: 288 x 1

  /+show Euler angles+/
\end{lstlisting}

%   As \textbf{oriAlpha} and \textbf{oriBeta} are suspect to crystal symmetry,
%   there are many symmetrically equivalent misorientations to \textbf{MO}.
...
\begin{frame}[fragile]
  \frametitle{Grain Exchange Symmetry}

  \begin{columns}
    \begin{column}{0.57\textwidth}
  \begin{itemize}
  \item consider misorientations \texttt{inv(ori1)*ori2} between orientations
    of the same phase
  \item sometimes no ordering \texttt{ori1}, \texttt{ori2} is given\\
    e.g. boundary misorientations
  \item $\rightarrow$ we should not distinguish between
    \texttt{inv(ori1)*ori2} \; vs \; \texttt{inv(ori2)*ori1}
  \item this additional symmetry is called grain exchange symmetry
  \item in MTEX it is denoted by the option \texttt{'antipodal'}
  \end{itemize}
  \begin{lstlisting}[style=input]
...
\vspace{-0.2cm}
\begin{lstlisting}[style=output]
ans = /+misorientation+/ (/+Ti (beta)+/ -> /+Ti (beta)+/)
  size: 8931 x 1
  antipodal: true
\end{lstlisting}
...
\begin{frame}[fragile]
  \frametitle{Misorientation Angle and Axis}

    \begin{columns}
    \begin{column}{0.48\textwidth}
      \begin{overlayarea}{\textwidth}{0.85\textheight}

        misorientation \vspace{-0.2cm}
        \begin{lstlisting}[style=input]
mori = inv(ori1)*ori2
    \end{lstlisting}
  ...
  the \structure{misorientation angle} is the smallest rotational angle of
  \textbf{mori.symmetrise}
...
angle(mori)
  ...
  the \structure{disorientation} is the misorientation with
  %\vspace{-0.5cm}
  \begin{itemize}
  \item minimum angle
  \item axis in the fundamental sector
  \end{itemize}
    \vspace{-0.2cm}
    \begin{lstlisting}[style=input]
mori.project2FundamentalRegion
    \end{lstlisting}

      \pause
  \medskip

  the rotational angle ignoring symmetry
    \vspace{-0.2cm}
    \begin{lstlisting}[style=input]
...
\begin{column}{0.48\textwidth}
  \begin{overlayarea}{\textwidth}{0.85\textheight}
\pause
  the rotational axis of \texttt{mori} refers to the disorientation
  \vspace{-0.2cm}
\begin{lstlisting}[style=input]
axis(mori,csAlpha)
axis(mori,csBeta)
...
ans = /+Miller+/ (/+Ti (alpha)+/)      
        h       k       i       l
  -1.5812  2.9291  -1.348  0.5158

ans = /+Miller+/ (/+Ti (beta)+/)
        h       k       l
  -0.1511  3.2874  0.3649
\end{lstlisting}
\end{onlyenv}
...
the axis in specimen coordinates
\vspace{-.3cm}
\begin{lstlisting}[style=input]
axis(ori1,ori2)
\end{lstlisting}
...
\end{lstlisting}
    ...
    the axis for the maximum rotational angle
    \vspace{-0.2cm}
    \begin{lstlisting}[style=input]
axis(mori,'max')
    \end{lstlisting}
...
\begin{frame}[fragile]
  \frametitle{Axis Angle Coloring}
  \begin{columns}
    \begin{column}{0.6\textwidth}

      the standard color key
      \vspace{-0.2cm}
      \begin{lstlisting}[style=input]
ori = ebsd('Fo').orientations
key = ipfHSVKey(ori.CS)
color = key.orientation2color(ori)
      \end{lstlisting}

      \pause
      \medskip

      sharp standard color key
      \vspace{-0.2cm}
      \begin{lstlisting}[style=input]
key.inversePoleFigureDirection =
      mean(ori)* key.whiteCenter
key.maxAngle = 4*degree
      \end{lstlisting}

      \pause
      \medskip

      sharp axis angle key
      \vspace{-0.2cm}
      \begin{lstlisting}[style=input]
key =  axisAngleKey
key.oriRef = mean(ori)
key.maxAngle = 4*degree
    ...
    \begin{column}{0.35\textwidth}
      ...
      \only<1>{\includegraphics[width=\textwidth]{pic/FoIpf}\\
        \vspace{-1.5cm}
        \includegraphics[width=0.4\textwidth]{pic/ipfKey}
      }
      \only<2>{
        \includegraphics[width=\textwidth]{pic/FoIpfSharp}\\
      \vspace{-1.5cm}
      \includegraphics[width=0.4\textwidth]{pic/ipfKeySharp}
    }

    \only<3>{
        \includegraphics[width=\textwidth]{pic/FoAxisAngle}\\
      \vspace{-0.5cm}
      \includegraphics[width=0.5\textwidth]{pic/aAkey}
}
    \end{column}
  \end{columns}
...
\begin{frame}[fragile]
  \frametitle{Grain Reference Orientation Deviation and mis2mean}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}
      \item \texttt{ori} - orientation at a certain pixel
      \item \texttt{oriG\_g}  - grain mean orientation
      \item \texttt{S} - symmetry to minimize the angle of
      \end{itemize}
      \begin{lstlisting}[style=input]
GROD = S * inv(ori_g) * ori
      \end{lstlisting}
...
\begin{lstlisting}[style=input]
axS = ebsd.orientations .* ...
  grod.axis('noSymmetry')
\end{lstlisting}
    ...
    \begin{column}{0.45\textwidth}
      ...
      \only<1>{\includegraphics[width=\textwidth]{pic/ebsdFe}}
      \only<2>{\includegraphics[width=\textwidth]{pic/ebsdFeGRODAngle}}
      \only<3>{\includegraphics[width=\textwidth]{pic/ebsdFeGOS}}
      \only<4>{\includegraphics[width=\textwidth]{pic/ebsdFeAx}\\
        \vspace{-0.75cm}%
        \includegraphics[width=0.25\textwidth]{pic/axKeyFe}}
      \only<5>{\includegraphics[width=\textwidth]{pic/ebsdFeAxS}\\
        \vspace{-0.75cm}%
        \includegraphics[width=0.5\textwidth]{pic/aAkey}}
...
\begin{frame}[fragile]
  ...
  \begin{columns}
    \begin{column}{0.6\textwidth}
      ...
      distribution of boundary misorientation angles
      \vspace{-0.2cm}
      \begin{lstlisting}[style=input]
mori = grains.boundary.misorientation
...
uniform misorientation distribution function
\vspace{-0.2cm}
      \begin{lstlisting}[style=input]
...
uncorrelated misorientation distribution function
\vspace{-0.2cm}
      \begin{lstlisting}[style=input]
odf = calcODF(ebsd.orientations)
umdf = calcMDF(odf)
plotAngleDistribution(umdf)
\end{lstlisting}

\pause
\medskip

distribution of boundary misorientations
\vspace{-0.2cm}
\begin{lstlisting}[style=input]
bmdf = calcDensity(mori)
...
\end{lstlisting}
    ...
    \begin{column}{0.38\textwidth}
      ...
      \begin{onlyenv}<1>
        \includegraphics[width=\textwidth]{pic/angleHist.pdf}
      \end{onlyenv}
      \begin{onlyenv}<2>
        \includegraphics[width=\textwidth]{pic/angleUMDF.pdf}
      \end{onlyenv}
      \begin{onlyenv}<3>
        \includegraphics[width=\textwidth]{pic/angleMDF.pdf}
      \end{onlyenv}
            \begin{onlyenv}<4>
        \includegraphics[width=\textwidth]{pic/angleFull.pdf}
      \end{onlyenv}
  \end{column}
  \end{columns}
...
\begin{frame}[fragile]
  \frametitle{Misorientation Axis Distribution}
...
\begin{column}{0.4\textwidth}
  \begin{overlayarea}{\textwidth}{0.85\textheight}
    \begin{onlyenv}<1>
      \includegraphics[width = \textwidth]{pic/axisScatter}
    \end{onlyenv}

    \begin{onlyenv}<2->
      \includegraphics[width = \textwidth]{pic/axisSmooth}
    \end{onlyenv}

    \begin{onlyenv}<3>
      \includegraphics[width = \textwidth]{pic/axisCS}
    \end{onlyenv}
        \begin{onlyenv}<4>
      \includegraphics[width = \textwidth]{pic/axisMDF}
    \end{onlyenv}
  \end{overlayarea}
...
\begin{column}{0.23\textwidth}
  \begin{overlayarea}{\textwidth}{0.85\textheight}
    \vspace{-0.75cm}
\centering
\only<3>{\includegraphics[width=\textwidth]{pic2/varPf1}}
\only<4>{\includegraphics[width=\textwidth]{pic2/varPf2}}
\only<5>{\includegraphics[width=\textwidth]{pic2/varPf3}}
\only<6>{\includegraphics[width=\textwidth]{pic2/varPf4}}
\only<7>{\includegraphics[width=\textwidth]{pic2/varPf5}}
\only<8>{\includegraphics[width=\textwidth]{pic2/varPf6}}
\only<9>{\includegraphics[width=\textwidth]{pic2/varPf7}}
\only<10>{\includegraphics[width=\textwidth]{pic2/varPf8}}
\only<11>{\includegraphics[width=\textwidth]{pic2/varPf9}}
\only<12>{\includegraphics[width=\textwidth]{pic2/varPf10}}
\only<13>{\includegraphics[width=\textwidth]{pic2/varPf11}}
\only<14>{\includegraphics[width=\textwidth]{pic2/varPf12}}
\only<15>{\includegraphics[width=\textwidth]{pic2/varPf}}
\end{overlayarea}
\end{column}
  \end{columns}
