...
tP = grains.triplePoints('iron','iron','iron');

% compute for each triple point the best fitting parentId and how well the fit is
tPori = grains(tP.grainId).meanOrientation;
[~, fit] = calcParent(tPori,fcc2bcc,'numFit',1,'id','threshold',5*degree);

plot(ebsd('Iron bcc'),ebsd('Iron bcc').orientations,'figSize','large')
hold on;
plot(gB,'linewidth',2,'smooth')
hold off
...
%parentGrains = parentGrains.smooth(5)

plot(parentGrains('Iron fcc'),parentGrains('Iron fcc').meanOrientation,'linewidth',2,'smooth')
